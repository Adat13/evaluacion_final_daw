...
def upload_silabo(seccion_id):
    ...
    # Secure the filename
    filename = secure_filename(f"silabo_{s.id}_{file.filename}")
    filepath = os.path.join(current_app.config['UPLOAD_FOLDER'], filename)
    file.save(filepath)

    # Update silabo_url
    s.silabo_url = f"/api/uploads/silabos/{filename}"
    db.session.add(s)
    ...
    log = AuditLog(
        ...
        action="SILABO_UPLOAD",
        ip_address=request.remote_addr,
        details=f"Docente subió el sílabo para la sección ID: {s.id}. Archivo: {filename}"
    )
    db.session.add(log)
    db.session.commit()

    return jsonify({
        'message': 'Sílabo cargado con éxito',
        'silabo_url': s.silabo_url
    }), 200
